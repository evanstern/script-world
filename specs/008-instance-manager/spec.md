...
**Input**: User description: "Instance manager for running worlds ('scriptworld ps'-style lifecycle management, docker/ollama ergonomics). Problem: users run multiple world daemons at once, lose track of them, and concurrent daemons clobber the shared local LLM host and step on each other. Proposal: (1) name-based world addressing with a default worlds home at ~/.scriptworld/worlds/<name> — `scriptworld new <worldname>` creates there by default; a --world=/path/to flag (or explicit path) still supports arbitrary locations, preserving the grounded 'one directory = one world, never global, cleanly separable' decision. (2) Manager subcommands: `scriptworld ps` lists running (and optionally all known) worlds; `stop <name>`, `start <name>`, `status <name>`, and existing per-world commands accept a world name resolved against the worlds home, in addition to paths. Existing path-based invocations must keep working."
...
**Independent Test**: Start two worlds by path (existing commands), run `scriptworld ps`
...
1. **Given** two world daemons running (started from any locations), **When** the user
   runs `scriptworld ps` from any working directory, **Then** both worlds are listed
   with name, state (running/paused), pid, tick, in-game time, speed, and LLM
   enabled/disabled — and the command exits 0.
2. **Given** no daemons running, **When** the user runs `scriptworld ps`, **Then** an
...
3. **Given** a daemon that was SIGKILLed (stale pidfile/socket left behind), **When**
   the user runs `scriptworld ps`, **Then** that world is not shown as running.
4. **Given** stopped worlds exist in the worlds home or were previously started,
   **When** the user runs `scriptworld ps --all`, **Then** stopped worlds are listed too,
...
A player creates a world with just a name — `scriptworld new aria` — and it lands in the
default worlds home (`~/.scriptworld/worlds/aria`). From then on, every command that
previously took a directory accepts the name: `scriptworld start aria`,
`scriptworld ui aria`, `scriptworld stop aria`. Players who want full control of
...
**Independent Test**: Run `scriptworld new testworld`, verify the directory appears
...
1. **Given** the worlds home does not yet exist, **When** the user runs
   `scriptworld new aria`, **Then** the worlds home is created, a complete world is
...
2. **Given** a world `aria` already exists in the worlds home, **When** the user runs
   `scriptworld new aria`, **Then** the command refuses with a clear error and exit
...
5. **Given** the user runs `scriptworld new` with a location override for a custom
...
6. **Given** a name that does not resolve to any known world, **When** a per-world
   ...
   it searched and suggests `scriptworld ps --all`.
...
`scriptworld ps` shows it by name and `scriptworld stop <name>` stops it; confirm the
...
1. **Given** a world at a custom path started by path, **When** the user runs
   `scriptworld ps`, **Then** the world appears under its recorded name with its data.
2. **Given** that world running, **When** the user runs `scriptworld stop <name>`,
...
- **FR-004**: `new <worldname>` MUST create a world named `<worldname>` in the default
  worlds home (`~/.scriptworld/worlds/<worldname>`), creating the worlds home on first
...
- **Worlds home**: The default parent directory (`~/.scriptworld/worlds`) where
...
- **Worlds home default** is `~/.scriptworld/worlds`, overridable via an environment
