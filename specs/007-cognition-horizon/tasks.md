...
- [x] T012 [US1] Implement `promptworld calibrate` in cmd/promptworld per contracts/cli.md (reference workload musing-1pt/planner-3pt, medians, writes calibration.json, human summary including the per-speed cognition-horizon verdict line, exit codes, cloud opt-in) with mock-provider test
