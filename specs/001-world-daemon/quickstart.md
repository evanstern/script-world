...
```sh
scriptworld new /tmp/w1 --name demo --seed 42
scriptworld start /tmp/w1            # detached daemon
scriptworld status /tmp/w1           # tick advancing, speed 4x
sleep 60
scriptworld status /tmp/w1           # ~4 game-minutes later (4x: 60s real ≈ 240 game-s)
scriptworld attach /tmp/w1           # header + live event stream; Ctrl-C to detach
scriptworld status /tmp/w1           # still running — detach ≠ pause
```

## Scenario B — time is a dial (US2, SC-004/SC-005)

```sh
scriptworld pause /tmp/w1            # paused:true
scriptworld status /tmp/w1; sleep 30; scriptworld status /tmp/w1   # tick unchanged
scriptworld resume /tmp/w1
scriptworld speed /tmp/w1 1x         # 1 game-s per real-s
scriptworld speed /tmp/w1 max        # as fast as sustainable; watch effective_rate
scriptworld speed /tmp/w1 4x
```

## Scenario C — kill and lossless resume (US3, SC-003)

```sh
scriptworld tail /tmp/w1 --since 0 | tail -3     # note last seq
kill -9 "$(cat /tmp/w1/daemon.pid)"              # crash, no goodbye
scriptworld start /tmp/w1                        # recovers snapshot+log, < 10 s
scriptworld status /tmp/w1                       # clock continuous, same speed/pause state
scriptworld tail /tmp/w1 --since 0 | tail -3     # history intact, seq contiguous
```

## Scenario D — determinism (SC-006)

```sh
scriptworld new /tmp/da --seed 7 && scriptworld new /tmp/db --seed 7
...
```sh
scriptworld stop /tmp/w1
cp -R /tmp/w1 /tmp/w1-archive
scriptworld start /tmp/w1-archive    # the copy is a complete, runnable world
```
