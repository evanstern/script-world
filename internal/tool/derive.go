...
// hand-maintained vocabularies (goalVocabulary, validGoals, planGoals, and the
// per-verb gloss prose) whose drift this feature exists to kill.
//
...
// order — byte-identical to the old goalVocabulary const.
...
// set (replaces validGoals). A fresh map per call; callers on a hot path cache
// it once.
...
// plan-step accept set (replaces planGoals). The FR-012 drift cure lives in the
// catalog data (every World tool carries PlanStep: true), not in code here.
